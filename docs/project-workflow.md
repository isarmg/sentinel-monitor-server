...
队列合并为下一批请求，并读取最新筛选条件。“仅显示未确认事件”切换后立即排队刷新。
...
`native/build.sh` 只接受干净且 annotated `v0.2.18` 指向 HEAD 的 checkout、Linux x86_64 builder，以及与
...
## 9. Web 构建与 Foundation 0.9.0 流程

```text
package.json + package-lock.json 精确锁定 Foundation 0.9.0 和工具链
  -> npm ci
  -> check:foundation
...
构建期八个 Web 包来自 Foundation `v0.9.0` GitHub Release 归档，并由 lockfile integrity 锁定字节；
Rust crate 由版本 `=0.9.0` 和 revision `b146afefb1e864de78d5f6ac43cab7d1a4e02fc7` 双重锁定。两者都不读取
...
Rust/Axum
├─ sarmg-contracts 0.9.0 -> Administrator 认证 DTO/路径和跨语言合同
├─ sarmg-error 0.9.0 -> 严格 ErrorEnvelope/ErrorCode
├─ sarmg-schema-identity 0.9.0 -> metadata DDL/列、指纹 framing 与 exact identity
├─ sarmg-server-target 0.9.0 -> 编译期 x86_64-unknown-linux-gnu 门禁
