...
## 9. Web 构建与 Foundation 0.8.9 流程

```text
package.json + package-lock.json 精确锁定 Foundation 0.8.9 和工具链
  -> npm ci
  -> check:foundation
...
构建期八个 Web 包来自 Foundation `v0.8.9` GitHub Release 归档，并由 lockfile integrity 锁定字节；
Rust crate 由版本 `=0.8.9` 和 revision `2908f7598571a47c1469bfa08a08c4776cc93049` 双重锁定。两者都不读取
...
Rust/Axum
├─ sarmg-contracts 0.8.9 -> Administrator 认证 DTO/路径和跨语言合同
├─ sarmg-error 0.8.9 -> 严格 ErrorEnvelope/ErrorCode
├─ sarmg-schema-identity 0.8.9 -> metadata DDL/列、指纹 framing 与 exact identity
├─ sarmg-server-target 0.8.9 -> 编译期 x86_64-unknown-linux-gnu 门禁
