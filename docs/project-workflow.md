...
`native/build.sh` 只接受干净且 annotated `v0.2.13` 指向 HEAD 的 checkout、Linux x86_64 builder，以及与
...
## 9. Web 构建与 Foundation 0.8.2 流程

```text
package.json + package-lock.json 精确锁定 Foundation 0.8.2 和工具链
  -> npm ci
  -> check:foundation
