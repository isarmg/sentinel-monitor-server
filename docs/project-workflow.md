...
`native/build.sh` 只接受干净且 annotated `v0.2.11` 指向 HEAD 的 checkout、Linux x86_64 builder，以及与
...
## 9. Web 构建与 Foundation 0.8.1 流程

```text
package.json + package-lock.json 精确锁定 Foundation 0.8.1 和工具链
  -> npm ci
  -> check:foundation
...
构建期八个 Web 包来自 Foundation `v0.8.1` GitHub Release 归档，并由 lockfile integrity 锁定字节；
