...
```text
/opt/isarmg/sentinel-monitor/releases/0.2.14/
├─ RELEASE-MANIFEST
...
/opt/isarmg/sentinel-monitor/releases/0.2.14/native/sentinelctl bootstrap
sudoedit /etc/isarmg/sentinel-monitor.env
/opt/isarmg/sentinel-monitor/releases/0.2.14/native/sentinelctl bootstrap --confirm-config
/opt/isarmg/sentinel-monitor/releases/0.2.14/native/sentinelctl start
/opt/isarmg/sentinel-monitor/releases/0.2.14/native/sentinelctl status
```
...
```bash
/opt/isarmg/sentinel-monitor/releases/0.2.14/native/sentinelctl stop
```
...
"/opt/isarmg/sentinel-monitor/releases/0.2.14/bin/sentinel-monitor" doctor --offline
/opt/isarmg/sentinel-monitor/releases/0.2.14/native/sentinelctl start
"/opt/isarmg/sentinel-monitor/releases/0.2.14/bin/sentinel-monitor" doctor
```
...
- 当前 Rust 固定 Foundation `=0.8.2` / `e349d8a3b63b6d9f2c41d1515a4909ce8e9821a5`，八个 Web 包固定同版正式 Release tarball 和 lockfile integrity，不读取相邻工作区。
