...
```text
/opt/isarmg/sentinel-monitor/releases/0.2.17/
├─ RELEASE-MANIFEST
...
/opt/isarmg/sentinel-monitor/releases/0.2.17/native/sentinelctl bootstrap
sudoedit /etc/isarmg/sentinel-monitor.env
/opt/isarmg/sentinel-monitor/releases/0.2.17/native/sentinelctl bootstrap --confirm-config
/opt/isarmg/sentinel-monitor/releases/0.2.17/native/sentinelctl start
/opt/isarmg/sentinel-monitor/releases/0.2.17/native/sentinelctl status
```
...
```bash
/opt/isarmg/sentinel-monitor/releases/0.2.17/native/sentinelctl stop
```
...
"/opt/isarmg/sentinel-monitor/releases/0.2.17/bin/sentinel-monitor" doctor --offline
/opt/isarmg/sentinel-monitor/releases/0.2.17/native/sentinelctl start
"/opt/isarmg/sentinel-monitor/releases/0.2.17/bin/sentinel-monitor" doctor
```
...
- 当前 Rust 固定 Foundation `=0.8.8` / `946bbb5f4bf3e06d421b6f463bd637b83617a5be`，八个 Web 包固定同版正式 Release tarball 和 lockfile integrity，不读取相邻工作区。
