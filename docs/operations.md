...
```text
/opt/isarmg/sentinel-monitor/releases/0.2.11/
├─ RELEASE-MANIFEST
...
/opt/isarmg/sentinel-monitor/releases/0.2.11/native/sentinelctl bootstrap
sudoedit /etc/isarmg/sentinel-monitor.env
/opt/isarmg/sentinel-monitor/releases/0.2.11/native/sentinelctl bootstrap --confirm-config
/opt/isarmg/sentinel-monitor/releases/0.2.11/native/sentinelctl start
/opt/isarmg/sentinel-monitor/releases/0.2.11/native/sentinelctl status
```
...
```bash
/opt/isarmg/sentinel-monitor/releases/0.2.11/native/sentinelctl stop
```
...
"/opt/isarmg/sentinel-monitor/releases/0.2.11/bin/sentinel-monitor" doctor --offline
/opt/isarmg/sentinel-monitor/releases/0.2.11/native/sentinelctl start
"/opt/isarmg/sentinel-monitor/releases/0.2.11/bin/sentinel-monitor" doctor
```
...
- 当前 Rust 固定 Foundation `=0.8.1` / `85348eb99cbea7798a2a3f8ea55baf0179708322`，八个 Web 包固定同版正式 Release tarball 和 lockfile integrity，不读取相邻工作区。
