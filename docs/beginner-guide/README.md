...
当前 key ID 固定为 `sentinel-credentials-0.2.14-key-1`。产品没有 previous key/keyring，不接受旧
...
`sarmg-schema-identity 0.8.2`；Sentinel 只保留 rusqlite 私有 generation adapter、文件安全和媒体租约等
