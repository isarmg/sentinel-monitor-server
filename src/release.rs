...
pub(crate) const PRODUCTION_RELEASE_ROOT: &str = "/opt/isarmg/sentinel-monitor/releases/0.2.16";
const RELOCATABLE_RELEASE_SUFFIX: &str = "opt/isarmg/sentinel-monitor/releases/0.2.16";
...
fn validate_release_root(root: &Path) -> Result<PathBuf> {
    ...
    let canonical = fs::canonicalize(root).context("resolve RELEASE_ROOT")?;
        ...
        "RELEASE_ROOT must end in the fixed Sentinel 0.2.16 physical release path"
    );
    Ok(canonical)
}
...
mod tests {
    ...
    fn source_revision_and_hashes_have_exact_encodings() {
        ...
        assert!(!is_source_revision("v0.2.16"));
