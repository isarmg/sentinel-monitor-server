...
Sentinel Monitor `0.2.20` 是自托管的浏览器摄像头监控系统。Rust/Axum 控制面负责管理员、Client 实例、摄像头状态、PTZ 和录像索引；固定版本的 MediaMTX companion 负责视频接入、播放与 Server 侧录像。
...
```sh
sudo /opt/isarmg/sentinel-monitor/releases/0.2.20/native/sentinelctl bootstrap
sudoedit /etc/isarmg/sentinel-monitor.env
sudo /opt/isarmg/sentinel-monitor/releases/0.2.20/native/sentinelctl bootstrap --confirm-config
sudo /opt/isarmg/sentinel-monitor/releases/0.2.20/native/sentinelctl start
sudo /opt/isarmg/sentinel-monitor/releases/0.2.20/native/sentinelctl status
```
