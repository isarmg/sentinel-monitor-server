...
refresh_static_contract
run_build >/dev/null
[[ -d "$INSTALL_ROOT/releases/0.2.9" && ! -L "$INSTALL_ROOT/releases/0.2.9" ]] ||
...
[[ -x "$INSTALL_ROOT/releases/0.2.9/native/sentinelctl" ]] || fail "release lifecycle command is missing"
[[ ! -x "$INSTALL_ROOT/releases/0.2.9/native/.start-action.sh" ]] ||
...
[[ -z "$(find -P "$INSTALL_ROOT/releases/0.2.9" -perm /222 -print -quit)" ]] ||
...
ln -s -- releases/0.2.9 "$INSTALL_ROOT/current"
...
if run_operation "$INSTALL_ROOT/releases/0.2.9/bin/sentinel-monitor" serve \
...
FIRST_MANIFEST="$(sha256sum "$INSTALL_ROOT/releases/0.2.9/RELEASE-MANIFEST" | awk '{print $1}')"
FIRST_LAYOUT="$(find -P "$INSTALL_ROOT" -printf '%P %y %m %s\n' | LC_ALL=C sort | sha256sum | awk '{print $1}')"
if run_build >"$TEST_ROOT/second-build.out" 2>&1; then
  fail "publisher accepted a second Sentinel 0.2.9 publication"
fi
grep -q 'one-shot' "$TEST_ROOT/second-build.out" ||
  fail "second publication did not identify the one-shot boundary"
[[ "$(sha256sum "$INSTALL_ROOT/releases/0.2.9/RELEASE-MANIFEST" | awk '{print $1}')" == "$FIRST_MANIFEST" ]] ||
...
if env \
  ...
  SENTINEL_NATIVE_STATE_DIR="$STATE_ROOT" \
  SENTINEL_NATIVE_RUNTIME_DIR="$RUNTIME_ROOT" \
  "$INSTALL_ROOT/releases/0.2.9/native/sentinelctl" bootstrap >"$TEST_ROOT/symlink-config.out" 2>&1; then
...
if env \
  ...
  SENTINEL_NATIVE_STATE_DIR="$STATE_ROOT" \
  SENTINEL_NATIVE_RUNTIME_DIR="$RUNTIME_ROOT" \
  "$INSTALL_ROOT/releases/0.2.9/native/sentinelctl" bootstrap >"$TEST_ROOT/symlink-env.out" 2>&1; then
  fail "bootstrap accepted a symbolic-link environment file"
fi

BOOTSTRAP_OUTPUT="$TEST_ROOT/bootstrap.out"
run_operation "$INSTALL_ROOT/releases/0.2.9/native/sentinelctl" bootstrap >"$BOOTSTRAP_OUTPUT"
ENV_FILE="$CONFIG_ROOT/sentinel-monitor.env"
[[ "$(stat -c '%a' "$ENV_FILE")" == "600" ]] || fail "environment file is not mode 0600"
grep -q '^STATIC_DIR=.*/releases/0.2.9/web$' "$ENV_FILE" || fail "STATIC_DIR is not release-pinned"
...
run_operation "$INSTALL_ROOT/releases/0.2.9/native/sentinelctl" bootstrap >/dev/null
[[ "$(sha256sum "$ENV_FILE" | awk '{print $1}')" == "$ENV_DIGEST" ]] ||
  fail "bootstrap overwrote an existing environment file"
if run_operation "$INSTALL_ROOT/releases/0.2.9/native/sentinelctl" start >"$TEST_ROOT/unconfirmed.out" 2>&1; then
  fail "start accepted an unconfirmed generated administrator password"
fi

sed -i 's/^BOOTSTRAP_ADMIN_PASSWORD=.*/BOOTSTRAP_ADMIN_PASSWORD=operator-reviewed-password-0.2.9/' "$ENV_FILE"
...
chmod 0600 -- "$ENV_FILE"
run_operation "$INSTALL_ROOT/releases/0.2.9/native/sentinelctl" bootstrap --confirm-config >/dev/null
...
if run_operation env \
  ...
  "$INSTALL_ROOT/releases/0.2.9/native/sentinelctl" start >"$TEST_ROOT/readiness-failure.out" 2>&1; then
...
if run_operation "$INSTALL_ROOT/releases/0.2.9/native/sentinelctl" start >"$TEST_ROOT/operation-lock.out" 2>&1; then
...
run_operation "$INSTALL_ROOT/releases/0.2.9/native/sentinelctl" start >/dev/null
for _ in {1..40}; do
...
STATUS_OUTPUT="$(run_operation "$INSTALL_ROOT/releases/0.2.9/native/sentinelctl" status)"
...
run_operation "$INSTALL_ROOT/releases/0.2.9/native/sentinelctl" stop >/dev/null
STATUS_OUTPUT="$(run_operation "$INSTALL_ROOT/releases/0.2.9/native/sentinelctl" status)"
...
RELEASE_ROOT="$INSTALL_ROOT/releases/0.2.9"
...
if run_operation "$INSTALL_ROOT/releases/0.2.9/native/sentinelctl" status >"$TEST_ROOT/hardlink.out" 2>&1; then
