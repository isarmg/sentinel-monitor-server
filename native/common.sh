...
readonly SENTINEL_VERSION="0.2.20"
...
resolve_release_context() {
  ...
  [[ "$SENTINEL_RELEASE_ROOT" == */opt/isarmg/sentinel-monitor/releases/0.2.20 ]] ||
    die "Operational scripts must use the fixed Sentinel 0.2.20 physical release suffix"
...
require_runtime_contract() {
  [[ "${DATABASE_URL:-}" == "sqlite://$SENTINEL_STATE_DIR/db/app.db" ]] ||
    die "DATABASE_URL must target the new 0.2.20 state directory"
  ...
  [[ "${RECORDINGS_DIR:-}" == "$SENTINEL_STATE_DIR/recordings" ]] ||
    die "RECORDINGS_DIR must target the new 0.2.20 state directory"
