...
if [[ "$MODE" == "confirm" ]]; then
  ...
  echo "Sentinel $SENTINEL_VERSION configuration accepted. Start it with: $SENTINEL_RELEASE_ROOT/native/sentinelctl start"
  exit 0
fi
...
(
  umask 077
  {
    ...
    echo "MEDIAMTX_RTSP_CERT=$SENTINEL_CONFIG_DIR/sentinel-rtsp.crt"
    echo "MEDIAMTX_RTSP_KEY=$SENTINEL_CONFIG_DIR/sentinel-rtsp.key"
